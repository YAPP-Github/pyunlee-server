...
class ProductCommentController(
    ...
    fun getProductCommentList(
        ...
        val result = productCommentProcessor.getCommentDetailList(productId, memberId, productCommentSearchDTO.toVO())
    ...
    fun likeComment(@PathVariable commentId: Long) {
        productCommentRatingProcessor.likeComment(memberId, commentId)
    }

    @PostMapping("/comment/{commentId}/cancel")
    @Operation(summary = "해당 코멘트 좋아요를 취소합니다.")
    fun cancelCommentLike(@PathVariable commentId: Long) {
        productCommentRatingProcessor.cancelLikeComment(memberId, commentId)
    }
}